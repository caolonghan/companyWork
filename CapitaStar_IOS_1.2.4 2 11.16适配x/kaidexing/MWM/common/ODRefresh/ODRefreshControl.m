...
@interface ODRefreshControl ()<UIScrollViewDelegate>
...
- (id)initInScrollView:(UIScrollView *)scrollView activityIndicatorView:(UIView *)activity
{
    ...
    if (self) {
        ...
        self.refreshState = GYRefreshStateIdle;
        ...
        _tintColor = [UIColor colorWithRed:54.0 / 255.0 green:54.0 / 255.0 blue:54.0 / 255.0 alpha:0.6];
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            if (self.scrollView.isDragging) { // 如果正在拖拽
                ...
                if (self.refreshState == GYRefreshStateIdle && offsetY < happenOffsetY) {
                    // 转为即将刷新状态
                    self.refreshState = GYRefreshStatePulling;
                    
                } else if (self.refreshState == GYRefreshStatePulling && offsetY >= 0) {
                    // 转为普通状态
                    self.refreshState = GYRefreshStateIdle;
                }
            } else if (self.refreshState == GYRefreshStatePulling) {// 即将刷新 && 手松开
                // 开始刷新
                self.refreshState = GYRefreshStateIdle;
                if (self.block!=nil)
                 {
