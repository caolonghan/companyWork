...
{
    [self presentPopupView:popupViewController.view animationType:animationType backgroundClickable:backgroundClickable];
}
...
- (void)presentPopupView:(UIView*)popupView animationType:(MJPopupViewAnimation)animationType backgroundClickable:(BOOL) backgroundClickable
{
    UIView *sourceView = [self topView];
...
- (void)slideViewOut:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView withAnimationType:(MJPopupViewAnimation)animationType
{
    ...
    [UIView animateWithDuration:kPopupModalAnimationDuration delay:0.0f options:UIViewAnimationCurveEaseIn animations:^{
        ...
        if([self respondsToSelector:@selector(popupViewOutEnd)]){
            [self performSelector:@selector(popupViewOutEnd)];
        }
    }];
}
