...
- (void)validateLayout
{
    ...
    if (self.messageLabel) {
        ...
        self.messageLabel.frame = CGRectMake(16, y, self.containerView.bounds.size.width - 16 * 2, height);
        y += height;
    }
    
    //增加一条横线
    
    UIView* lineView = [[UIView alloc] initWithFrame:CGRectMake(0, y, self.containerView.bounds.size.width, 1)];
    [lineView setBackgroundColor:[UIColor colorWithWhite:0.95 alpha:1]];
    [self.containerView addSubview:lineView];
    
    if(self.buttons.count == 2){
        lineView = [[UIView alloc] initWithFrame:CGRectMake(self.containerView.bounds.size.width/2, y, 1, BUTTON_HEIGHT)];
        [lineView setBackgroundColor:[UIColor colorWithWhite:0.95 alpha:1]];
        [self.containerView addSubview:lineView];
    }
...
- (void)updateMessageLabel
{
    if (self.message) {
        if (!self.messageLabel) {
            self.messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(16, 0, self.bounds.size.width - 32, self.bounds.size.height)];
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
	...
	switch (item.type) {
		...
		default:
			normalImage = [UIImage imageNamed:@"sialertview_button_pure"];
			highlightedImage = [UIImage imageNamed:@"sialertview_button_pure_on"];
