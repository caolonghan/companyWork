...
@property (nonatomic, strong) NSDictionary * attributesHashtagVip;
...
- (void)setupLabel {
    ...
    _attributesText = @{NSForegroundColorAttributeName: self.textColor, NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:15.0]};
    _attributesHandle = @{NSForegroundColorAttributeName:DEFAULT_MAIN_COLOR, NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:15.0]};
    _attributesHashtag = @{NSForegroundColorAttributeName: DEFAULT_MAIN_COLOR, NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:15.0]};
    _attributesLink = @{NSForegroundColorAttributeName: DEFAULT_MAIN_COLOR, NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:15.0]};
    
    _attributesHashtagVip = @{NSForegroundColorAttributeName: [UIColor redColor], NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:15.0]};
...
- (void)determineHotWords {
    ...
    while ([tmpText rangeOfCharacterFromSet:hotCharactersSet].location < tmpText.length) {
        ...
        if (length > 1)
            [_rangesOfHotWords addObject:@{@"hotWord": @(hotWord), @"range": [NSValue valueWithRange:NSMakeRange(range.location, length+1)],@"word":[tmpText substringWithRange:NSMakeRange(range.location, length+1)]}];
    }
...
- (void)updateText
{
    ...
    for (NSDictionary *dictionary in _rangesOfHotWords)  {
        ...
        NSString* word = [[dictionary valueForKey:@"word"] stringByReplacingOccurrencesOfString:@"#" withString:@""];
        word = [word stringByReplacingOccurrencesOfString:@"%" withString:@""];
        if([_vipWords containsObject:word]){
            [attributedString setAttributes:_attributesHashtagVip range:range];
        }else{
