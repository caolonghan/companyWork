...
-(UIView *)dailyInfoSubViewContainer
{
    ...
    line2View.frame = CGRectMake(0, 0, _dailyInfoSubViewContainer.frame.size.width, 1);
    line2View.backgroundColor = DEFAULT_COLOR_EEE;
    [_dailyInfoSubViewContainer addSubview:line2View];
...
{
    if(!_dateInfoLabel){
        _dateInfoLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_FRAME.size.width, DATE_LABEL_INFO_HEIGHT)];
...
-(void)initDailyViews
{
    ...
    UIView* lineView = [[UIView alloc] initWithFrame:CGRectMake(0, self.dayTitleSubViewContainer.frame.size.height - 1, SCREEN_FRAME.size.width, 1)];
    lineView.backgroundColor = DEFAULT_COLOR_EEE;
    [self.dayTitleSubViewContainer addSubview:lineView];
...
-(UILabel *)dayTitleViewForDate: (NSDate *)date inFrame: (CGRect)frame
{
    ...
    dayTitleLabel.text = [[[date getDayOfWeekShortString] uppercaseString] substringFromIndex:1];
...
-(void)adjustDailyInfoLabelAndWeatherIcon: (BOOL)blnShowWeatherIcon
{
    ...
    if(blnShowWeatherIcon){
        ...
        self.dateInfoLabel.frame = CGRectMake( 0, 0, SCREEN_FRAME.size.width, DATE_LABEL_INFO_HEIGHT);
    }
