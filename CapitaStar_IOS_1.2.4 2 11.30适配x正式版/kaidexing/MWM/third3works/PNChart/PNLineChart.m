...
#define YLABEL_TAG 9021001
#define XLABEL_TAG 9021002
//------------------------------------------------------------------------------------------------
...
-(void)setYLabels:(NSArray *)yLabels
{
	...
	while (num > 0) {
        ...
        label.tag = YLABEL_TAG;
        
        float showValue = level * index;
        if(showValue < 1000){
            label.text = [NSString stringWithFormat:@"%d",(int)showValue];
        }else if(showValue < 1000000){
            label.text = [NSString stringWithFormat:@"%1.fK",showValue / 1000];
        }else {
            label.text = [NSString stringWithFormat:@"%dM",(int)(showValue / 1000000)];
        }
...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    if(_showLabel){
        ...
        for(int index = 0; index < xLabels.count; index++)
        {
            ...
            PNChartLabel * label = [[PNChartLabel alloc] initWithFrame:CGRectMake(index * _xLabelWidth + 30.0, self.frame.size.height - 30.0, _xLabelWidth, _xLabelHeight)];
            [label setTextAlignment:NSTextAlignmentCenter];
            label.text = labelText;
            label.tag = XLABEL_TAG;
            [self addSubview:label];
        }
...
-(void)strokeChart
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        [progressline moveToPoint:CGPointMake( xPosition, _chartCavanHeight - grade * _chartCavanHeight + _xLabelHeight + yLabelHeight)];
        [linePointsArray addObject:[NSValue valueWithCGPoint:CGPointMake( xPosition, _chartCavanHeight - grade * _chartCavanHeight + _xLabelHeight + yLabelHeight)]];
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            if (index != 0) {
                CGPoint point = CGPointMake(index * _xLabelWidth + 30.0 + _xLabelWidth / 2.0, _chartCavanHeight - (innerGrade * _chartCavanHeight) + _xLabelHeight + yLabelHeight);
