...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _imgScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0,0,frame.size.width, frame.size.height )]; //+M_WIDTH(100)
        ...
        _pageControl.pageIndicatorTintColor = UIColorFromRGB(0xd9d9d9);
...
-(void)reloadView {
    ...
    for (int i = 0; i <= (self.pics.count == 1 ? self.pics.count - 1 : self.pics.count); i++) {
        ...
        NSString *urlStr = @"";
        if (i == 0) {
            urlStr = self.pics[self.pics.count - 1][@"img"];
        }else {
            urlStr = self.pics[i - 1][@"img"];
        }
        [self loadImage:urlStr imageView:imageView];
        [self setVFrame:i imageView:imageView];
        [_imgScrollView addSubview:imageView];
        
        UILabel *lab = [[UILabel alloc]initWithFrame:CGRectMake(self.frame.size.width * i,self.frame.size.width/2,self.frame.size.width-M_WIDTH(10),M_WIDTH(37))];
        if (i == 0) {
            lab.text=[Util isNil:self.pics[self.pics.count - 1][@"title"]];
        }else {
            lab.text=[Util isNil:self.pics[i-1][@"title"]];
        }
        [_imgScrollView addSubview:lab];
...
- (void)setBttonFrame:(int)index withButton:(UIButton *)sender{
    sender.frame = CGRectMake(self.frame.size.width * index, 0, self.frame.size.width,self.frame.size.width/2);
}
#pragma mark 点击事件
...
- (void)setVFrame:(int)index imageView:(UIImageView *)image{
    image.frame = CGRectMake(self.frame.size.width * index, 0, self.frame.size.width,self.frame.size.width/2);
}
#pragma mark 图片滚轮的方法
