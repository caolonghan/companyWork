...
@interface AddMyMallViewController ()<UITableViewDelegate,UITableViewDataSource>

@end

@implementation AddMyMallViewController{
    UIView *nilView1;
...
-(void)NetWorkRequest:(int)type{
    ...
    [HttpClient requestWithMethod:@"POST" path:[Util makeRequestUrl:@"mallshoplist" tp:@"loadmalllist"] parameters:dicton  target:self success:^(NSDictionary *dic){
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            dataAry=dic[@"data"][@"nofouce_malllist"];
...
-(UIView*)createCellView:(NSIndexPath*)index{
    ...
    [cancelBtn setBackgroundImage:[UIImage imageNamed:@"add_love"] forState:UIControlStateNormal];
...
-(UIView*)createButtomView{
    ...
    [okBtn setTitle:@"返回关注列表" forState:UIControlStateNormal];
...
-(void)cellTouch:(UIButton*)sender{
    ...
    [HttpClient requestWithMethod:@"POST" path:[Util makeRequestUrl:@"collection" tp:@"attention"] parameters:dicton  target:self success:^(NSDictionary *dic){
        dispatch_async(dispatch_get_main_queue(), ^{
            [SVProgressHUD dismiss];
